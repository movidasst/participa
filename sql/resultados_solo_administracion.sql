...
begin
  ...
  if not v_permitido then return jsonb_build_object('ok',false,'locked',true,'message','La administración comunicará los resultados al finalizar.'); end if;
...
$function$
;
update public.participa_consultas set resultados_visibilidad='admin_only',updated_at=now();
alter table public.participa_consultas alter column resultados_visibilidad set default 'admin_only';